    ...
    sections: [{
      ...
      children: [
          ...
          rows: [
              ...
              children: [
                ...
                new TableCell({
                  children: [new Paragraph({ 
                    text: handover.status === 'draft' ? 'Đang làm việc' :
                          handover.status === 'pending' ? 'Chờ xác nhận' :
                          handover.status === 'completed' ? 'Đã bàn giao' :
                          handover.status === 'rejected' ? 'Đã từ chối' : 'Không xác định'
                  })],
            ...
            new TableRow({
              children: [
                ...
                new TableCell({
                  children: [new Paragraph({ text: handover.FromShift?.name || 'Không xác định' })],
                }),
              ],
            }),
            new TableRow({
            ...
            new TableRow({
              children: [
                ...
                new TableCell({
                  children: [new Paragraph({ 
                    text: handover.updatedAt ? format(new Date(handover.updatedAt), 'HH:mm dd/MM/yyyy', { locale: vi }) : 'Chưa cập nhật'
                  })],
                }),
              ],
            }),
        ...
        new Table({
          ...
          rows: [
            ...
            ...defaultDevices.map((device, index) => {
              const deviceErrors = handover.devices?.filter(
              ...
              ...handover.Tasks.map(task => 
                new TableRow({
                  children: [
                    ...
                    new TableCell({
                      children: [new Paragraph({ text: task.taskTitle || 'Không có tiêu đề' })],
                    }),
                    new TableCell({
                      children: [new Paragraph({ text: task.fullName || 'Chưa phân công' })],
                    }),
                    new TableCell({
                      children: [new Paragraph({ 
                        text: task.status === 'in_progress' ? 'Đang thực hiện' :
                              task.status === 'waiting' ? 'Chờ xử lý' :
                              task.status === 'completed' ? 'Đã hoàn thành' :
                              task.status === 'cancelled' ? 'Đã hủy' : 'Không xác định'
                      })],
                    }),
                  ],
                })
        ...
        ...(deviceCheckData && deviceCheckData.length > 0 ? [
          ...
          ...deviceCheckData.map((checkForm, formIndex) => [
            new Paragraph({
              children: [
                new TextRun({
              ...
              rows: [
                ...
                new TableRow({
                  children: [
                    ...
                    new TableCell({
                      children: [new Paragraph({ 
                        text: checkForm.checkedAt ? format(new Date(checkForm.checkedAt), 'HH:mm dd/MM/yyyy', { locale: vi }) : 'Không có'
                      })],
                    }),
                  ],
                }),
                new TableRow({
                  children: [
                    ...
                    new TableCell({
                      children: [new Paragraph({ text: checkForm.checkerName || 'Không có' })],
                    }),
                  ],
                }),
              ],
            ...
            new Table({
              ...
              rows: [
                ...
                ...(checkForm.items || []).map((item, itemIndex) => 
                  new TableRow({
                    children: [
                      new TableCell({
                      ...
                      new TableCell({
                        children: [new Paragraph({ text: item.deviceNameSnapshot || `Thiết bị ${item.deviceId}` })],
                      }),
                      new TableCell({
                        children: [new Paragraph({ 
        ...
        ...(deviceCheckData && deviceCheckData.length > 0 ? [
          ...
          ...deviceCheckData.map((checkForm, index) => 
            new Paragraph({
