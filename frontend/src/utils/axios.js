...
const resolveQueue = (token) => { pendingQueue.forEach((res) => res(token)); pendingQueue = []; };

const attachRequestInterceptor = (client) => client.interceptors.request.use(
  (config) => {
...
const attachResponseInterceptor = (client) => client.interceptors.response.use(
  (response) => response,
  async (error) => {
