...
import { Modal, Table, Card, Typography, Space, Tag, Row, Col, Button, message, Badge, Spin } from 'antd';
...
const ShiftScheduleModal = ({ visible, onCancel }) => {
  ...
  const calculateCurrentCycle = (today) => {
    ...
    const daysDiff = Math.floor((currentMonday.getTime() - anchorMonday.getTime()) / (7 * 24 * 60 * 60 * 1000));
  ...
  const generateWeekData = (cycleStartMonday, weekIndex, today) => {
    ...
    days.forEach((day, dayIndex) => {
      ...
      if (cellDate.getDate() === today.getDate() &&
        ...
        console.log('🎯 Tìm thấy ngày hiện tại tại:', getWeekTitle(weekIndex), day, cellDate.toDateString());
      }
    });
  ...
  const calculateWeekDates = (today) => {
    console.log('🔍 Ngày hiện tại:', today.toDateString(), 'Date:', today.getDate(), 'Month:', today.getMonth() + 1);
    ...
    for (let weekIndex = 1; weekIndex <= 4; weekIndex++) {
      ...
      console.log(`🔍 ${getWeekTitle(weekIndex)} - Tuần ${weekInfo.weekNumber}/${weekInfo.year} (${weekInfo.startDate.getDate()}/${weekInfo.startDate.getMonth() + 1} - ${weekInfo.endDate.getDate()}/${weekInfo.endDate.getMonth() + 1})`);
    }

    // Hiển thị thông tin chu kỳ hiện tại
    console.log('🔄 Chu kỳ hiện tại:', cycle.cycleStartMonday.getDate() + '/' + (cycle.cycleStartMonday.getMonth() + 1),
      'đến', cycle.cycleEndDate.getDate() + '/' + (cycle.cycleEndDate.getMonth() + 1));
  ...
  const createColumns = useCallback(() => {
    ...
    const baseColumns = [
      {
        ...
        width: 15,
        align: 'center',
        render: (text) => <Text style={{ fontSize: '14px', fontWeight: '600', letterSpacing: '0.025em' }}>{text}</Text>
      },
      {
        title: 'Họ đệm',
        dataIndex: 'hoDem',
    ...
    weekColumns.forEach(week => {
      ...
      days.forEach(day => {
        ...
        const isTodayCell = dateInfo &&
          dateInfo.date === today.getDate() &&
          dateInfo.month === (today.getMonth() + 1);
  ...
  const isCurrentUserRow = useCallback((record) => {
    ...
    >
      ...
      <div style={{ marginBottom: 5, padding: '0 20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            {currentDate && (
              <div style={{ fontSize: '14px', color: '#ff4d4f', fontWeight: '600', marginTop: '2px' }}>
                <CalendarOutlined style={{ marginRight: '2px' }} />
                Hôm nay: {(() => {
                  const dayOfWeek = currentDate.getDay();
                  const dayNames = ['Chủ nhật', 'Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7'];
                  return dayNames[dayOfWeek];
                })()} ngày {currentDate.getDate()}/{currentDate.getMonth() + 1}/{currentDate.getFullYear()}
                <Button
                  type="primary"
                  size="small"
      ...
      {loading && !isInitialized ? (
        ...
        <>
          ...
          <div style={{ marginTop: 12, padding: '10px', backgroundColor: '#f8f9fa', borderRadius: '8px', border: '1px solid #e9ecef' }}>
            <div style={{ marginBottom: 12 }}>
              <Text strong style={{ fontSize: '14px', color: '#2D3958' }}>Ký hiệu ca trực:</Text>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', fontSize: '12px' }}>
              ...
              <div>
                <div style={{ marginBottom: 8 }}>
                  <Tag color={SHIFT_COLORS['T']} style={{ marginRight: 8, fontWeight: 'bold' }}>T</Tag>
                  <span>Làm việc từ 8h - 17h tại Tầng 3 -108 THĐ</span>
                </div>
                <div style={{ marginBottom: 8 }}>
                  <Tag color={SHIFT_COLORS['T1']} style={{ marginRight: 8, fontWeight: 'bold' }}>T1</Tag>
                  <span>Làm việc từ 6h - 14h tại Tầng 3 -108 THĐ</span>
                </div>
                <div style={{ marginBottom: 8 }}>
                  <Tag color={SHIFT_COLORS['T2']} style={{ marginRight: 8, fontWeight: 'bold' }}>T2</Tag>
                  <span>Làm việc từ 14h - 22h tại Tầng 3 -108 THĐ</span>
                </div>
                <div style={{ marginBottom: 8 }}>
                  <Tag color={SHIFT_COLORS['T3']} style={{ marginRight: 8, fontWeight: 'bold' }}>T3</Tag>
                  <span>Làm việc từ 22h - 06h tại Tầng 3 -108 THĐ</span>
                </div>
                <div style={{ marginBottom: 8 }}>
                  <Tag color={SHIFT_COLORS['N']} style={{ marginRight: 8, fontWeight: 'bold' }}>N</Tag>
              ...
              <div>
                <div style={{ marginBottom: 8 }}>
                  <Tag color={SHIFT_COLORS['H1']} style={{ marginRight: 8, fontWeight: 'bold' }}>H1</Tag>
                  <span>Làm việc từ 7h30 - 18h30 tại LHL</span>
                </div>
                <div style={{ marginBottom: 8 }}>
                  <Tag color={SHIFT_COLORS['H2']} style={{ marginRight: 8, fontWeight: 'bold' }}>H2</Tag>
                  <span>Làm việc từ 18h30 - 7h30 tại LHL</span>
                </div>
                <div style={{ marginBottom: 8 }}>
                  <Tag color={SHIFT_COLORS['V1']} style={{ marginRight: 8, fontWeight: 'bold' }}>V1</Tag>
                  <span>Làm việc từ 7h30 - 18h30 tại VC</span>
                </div>
                <div style={{ marginBottom: 8 }}>
                  <Tag color={SHIFT_COLORS['V2']} style={{ marginRight: 8, fontWeight: 'bold' }}>V2</Tag>
