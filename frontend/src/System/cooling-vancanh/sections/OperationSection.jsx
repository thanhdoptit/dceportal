...
const OperationSection = () => {
  ...
  const startupProcedure = [
    {
      ...
      notes:
        'Thứ tự Chiller sẽ được tính toán và cài đặt sao cho tổng thời gian hoạt động của các Chiller tương đương nhau. Chiller được cài đặt ở chế độ Remote để hệ thống BMS có thể can thiệp.',
    },
    {
      step: 2,
      ...
      notes:
        'Tham số cài đặt phải đảm bảo duy trì chênh lệch áp suất giữa đường ống tối thiểu vận hành của hệ thống.',
    },
    {
      step: 3,
      ...
      notes:
        'Vận hành ở chế độ nạp. Van V2A, V2B sẽ được điều tiết mở theo tải làm mát đòi hỏi của DC, phần trăm mở tối thiểu là 10%.',
    },
    {
      step: 4,
      ...
      notes:
        'Tham số cài đặt phải đảm bảo duy trì áp suất đường ống tối thiểu vận hành của hệ thống. Các bơm trong mỗi cụm được vận hành luân phiên sao cho thời gian hoạt động của các bơm là tương đương nhau.',
    },
  ];
  ...
  const normalOperation = [
    {
      ...
      parameters:
        'Tần số bơm nước lạnh điều chỉnh trong dải sao cho vẫn đảm bảo lưu lượng tối thiểu đi qua Chiller và đảm bảo được lưu lượng nước tối thiểu đi qua từng máy làm lạnh nước khi đang vận hành.',
    },
    {
      phase: 'Chế độ Normal',
      ...
      parameters:
        'Vận hành hệ thống van Bypass tiếp tục từ Quy trình khởi động. Vận hành hệ thống bơm bù tiếp tục từ Quy trình khởi động.',
    },
  ];
  ...
  const chillerControl = [
    ...
    },
    {
      ...
      notes:
        'Cụm Chiller nào bật trước sẽ được ra lệnh tắt trước khi có yêu cầu tắt từ hệ thống, sao cho thời gian hoạt động của các Chiller là tương đương nhau.',
    },
  ];
  ...
  const rotationOperation = {
    description:
      'Hệ thống BMS liên tục theo dõi trạng thái hoạt động của các thành phần trong hệ thống, từ đó tính toán vận hành luân phiên các cụm Chiller theo giờ, đảm bảo thời gian vận hành của các cụm là tương đương nhau.',
    parameters:
      'Giá trị cài đặt thời gian luân phiên có thể được cài đặt bởi người vận hành để phù hợp với thực tế vận hành của hệ thống. Giá trị cài đặt mặc định là 8h.',
  };
  ...
  const emergencyOperation = [
    {
      ...
      description:
        'Một thành phần bất kỳ của cụm bị lỗi: Máy lạnh Chiller bị lỗi hoặc không khởi động lên được, van chặn không mở được hoàn toàn, hoặc bộ điều khiển thuộc cụm Chiller bị lỗi.',
      actions: [
    ...
    },
    {
      ...
      notes:
        'Phải đảm bảo cung cấp nước lạnh lấy sẵn từ Két dự phòng Tes cung cấp cho tải lạnh của DC trong thời gian ~ 10 phút. Chỉ thực hiện sau khi đã khắc phục xong sự cố về mất điện nguồn Chiller và hệ thống đã hoạt động ổn định trở lại.',
    },
  ];
  ...
  const operationModes = [
    ...
    },
    {
      mode: 'Manual',
      description:
        'Chế độ vận hành trên giao diện vận hành, vận hành trực tiếp bởi cán bộ vận hành.',
      usage: 'Sử dụng khi cần can thiệp thủ công vào quá trình vận hành',
    },
    {
      mode: 'Auto',
      description:
        'Chế độ vận hành tự động, dựa trên thời gian hoạt động thực tế của thiết bị mà xác định thứ tự hoạt động của cả cụm thiết bị.',
    ...
    <div id='section-5' className='content-section'>
      ...
      <div id='section-5-1' className='subsection'>
        ...
        <Card title='Quy trình khởi động hệ thống làm mát' style={{ marginBottom: '20px' }}>
          <Paragraph>
            Quy trình khởi động hệ thống làm mát được thực hiện theo các bước tuần tự để đảm bảo an
            toàn và hiệu quả.
          </Paragraph>
      ...
      <div id='section-5-3' className='subsection'>
        ...
        <Card title='Quy trình điều khiển cụm Chiller' style={{ marginBottom: '20px' }}>
          <Paragraph>
            Hệ thống tự động điều khiển việc gọi thêm hoặc cắt bớt cụm Chiller dựa trên tải làm mát
      ...
      <div id='section-5-4' className='subsection'>
        ...
        <Paragraph style={{ marginBottom: '16px' }}>{rotationOperation.description}</Paragraph>
      ...
      <div id='section-5-5' className='subsection'>
        ...
        <Card title='Quy trình xử lý sự cố và chế độ khẩn cấp' style={{ marginBottom: '20px' }}>
          <Paragraph>
            Các quy trình xử lý sự cố và chế độ khẩn cấp để đảm bảo an toàn và ổn định của hệ thống
            làm mát.
          </Paragraph>
      ...
      <div id='section-5-6' className='subsection'>
        ...
        <Card title='Các chế độ vận hành hệ thống' style={{ marginBottom: '20px' }}>
          <Paragraph style={{ marginBottom: '16px' }}>
            Với mỗi cụm thiết bị sẽ được vận hành tuần tự theo thứ tự được định sẵn và có các chế độ
            vận hành riêng tùy theo yêu cầu vận hành của hệ thống, người vận hành cần phải thao tác
            chọn chế độ thích hợp trước khi vận hành hệ thống.
          </Paragraph>
