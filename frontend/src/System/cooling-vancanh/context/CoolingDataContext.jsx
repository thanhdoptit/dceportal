...
export const CoolingDataProvider = ({ children }) => {
  ...
  const loadSectionData = async (sectionName, delay = 200) => {
    ...
    console.log(`✅ ${sectionName} data loaded!`);
  };
  ...
  useEffect(() => {
    const loadAllDataSequentially = async () => {
      try {
        ...
        console.log('🚀 Bắt đầu lazy loading tất cả dữ liệu hệ thống làm mát...');
        ...
        const allSections = ['introduction', 'devices', 'pumps', 'location', 'operations', 'safety', 'documentation', 'contact'];
