...
import { 
  Table, Card, Button, Space, DatePicker, Input, 
  Tag, Modal, Typography, Descriptions, message 
} from 'antd';
import { 
          ...
          <div className="space-y-6">
            <Descriptions title="Thông tin cơ bản" bordered column={2}>
              ...
              <Descriptions.Item label="Trạng thái">
                <Tag color={
                  selectedHandover.status === 'confirmed' ? 'success' :
                  selectedHandover.status === 'rejected' ? 'error' : 'warning'
                }>
                  {
                    selectedHandover.status === 'confirmed' ? 'Đã xác nhận' :
                    selectedHandover.status === 'rejected' ? 'Từ chối' : 'Chờ xác nhận'
                  }
                </Tag>
              </Descriptions.Item>
            ...
            {selectedHandover.status === 'rejected' && selectedHandover.rejectReason && (
              <div>
                <Text strong type="danger">Lý do từ chối:</Text>
                <div className="mt-2 p-4 bg-red-50 rounded">
                  <pre className="whitespace-pre-wrap">
                    {selectedHandover.rejectReason}
                  </pre>
                </div>
              </div>
            )}
